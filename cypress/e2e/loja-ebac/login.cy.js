...
describe('Funcionalidade: Login', ( ) => {
    ...
    it('Deve fazer login com sucesso usando fixture', async() => {
        const perfilFixture = await cy.fixture('perfil')
       
            cy.get('#username').type(perfilFixture.usuario, {log: false})
            cy.get('#password').type(perfilFixture.senha, {log: false})
            cy.get('.woocommerce-form > .button').click()
            cy.get('.woocommerce-MyAccount-content > :nth-child(2)').should('contain', 'Olá, gabriel.ncimadon')
